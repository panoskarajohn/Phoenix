...
internal sealed class RabbitMqBackgroundService : BackgroundService
{
    ...
    public RabbitMqBackgroundService(IServiceProvider serviceProvider)
    {
        ...
        _exceptionToFailedMessageMapper =
            _serviceProvider.GetService<IExceptionToFailedMessageMapper>() ?? _exceptionFailedMapper;
            ...
            try
            {
                ...
                if (_loggerEnabled)
                {
                    ...
                    _logger.LogInformation("Received a message with ID: '{MessageId}', " +
                        ...
                        messageId, correlationId, timestamp, conventions.Queue, conventions.RoutingKey,
                        conventions.Exchange, messagePayload);
                }
